...
[FCC Challenge](https://learn.freecodecamp.org/javascript-algorithms-and-data-structures/intermediate-algorithm-scripting/wherefore-art-thou/)
...
```
  function whatIsInAName(collection, source) {
      ...
      return collection.filter(getProperties);
    }
...
[FCC Challenge](https://learn.freecodecamp.org/javascript-algorithms-and-data-structures/intermediate-algorithm-scripting/spinal-tap-case/)
...
// In this case we can use $operator in regular expression. 
// Two ranges of characters, a-z and A-Z. Characters from these 
// ranges are grouped together. So we need to identify the two 
//  characters which are taken from two respective ranges(For example: “sI”, “sS” //and “lT”) .
...
```
    function translatePigLatin(str) {
      ...
      // First piece, till one by the last letter.
      ...
      // If it starts with a consonant and the last is not a vowel
        ...
        // So if we log consonants.index is 0, and consonants.input is consonants.
...
```
    function translatePigLatin(str) {
      ...
      if (str[0].match(regex)) {
        ...
        // If we console.log(str.match(regex));
        // We get ["o", "o", "a"]
...
```
FCC basic Code Solution. But, go check the Intermediate too.
```
function fearNotLetter(str) {
...
I got the reduce solution from [stackoverflow](https://stackoverflow.com/questions/9229645/remove-duplicate-values-from-js-array)
...
function sumFibs(num) {
      ...
      for (var i = 0; i < arr.length; i++) {
        if (arr[i] % 2 !== 0 && arr[i] <= num) {
          sum += arr[i];
          arr1.push(arr[i]) // This is just for checking the numbers we get.
        }
      }
      console.log(arr1);
      return sum;
...
```
    ...
    function nextPrime(value) {
      ...
      // How is it possible to call value with an empty array?
      let value, result = [];
      for (let i = 0; i < num; i++) {
...
```
    ...
    function smallestCommons(arr) {
      ...
      // the loop will always break and the product will never be returned.
