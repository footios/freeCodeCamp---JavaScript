...
```
[Go up](#goUp)
> where do these red dots come from? They are not visible in Atom where I copied from.
> But if you try to backspace them, they take one stike too.
> So they are present but not visible.
```
    function convertHTML(str) {
      let arr = str.split('');
